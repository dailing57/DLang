            ...
            if self.dst.id == -1:
                ...
                if mem is not None:
                    if mem.type is None:
                        ...
                        raise Error(
                            f'{mem.type} variable {self.dst.name} can not be assigned {res.dst.type} value')
                    if self.type == Assign and mem.isConst:
                        raise Error(
                            f'const variable {self.dst.name} can not be assigned a new value')
                    dst = GlobalVariableCode(globalAddress=mem.id, type=mem.type) if mem.isGloabal else LocalVariableCode(
        ...
        if fn is not None:
            if len(self.args) == len(fn.args):
                ...
                if self.args.checkType(fn.args):
                    ...
                    raise(
                        Error(f'function "{self.name}" call arg list types are not matched'))
            else:
                raise(
                    Error(f'function "{self.name}" call arg list length is not matched'))
        else:
    ...
    def visit(self, context: Context) -> NodeVisitorReturn:
        ...
        if self.src is not None:
            ...
            if res.dst is not None:
                ...
                raise(
                    Error(f'function "{context.fnName}" return value is void'))
        else:
            if fnObj.type != voidType:
                raise(Error(
                    f'function "{context.fnName}" should return <{fnObj.type}>, but return <void>'))
            returnCode = FunctionReturnCode(
                type=ThreeAddressCodeType.FunctionReturn,
                name=context.fnName
            )
...
class ForStatementASTNode(BasicASTNode):
    ...
    def visit(self, context: Context) -> NodeVisitorReturn:
        ...
        context = Context(
            symbols=sTable, globalFns=context.globalFns, fnName=context.fnName)
        code: list[ThreeAddressCode] = []
